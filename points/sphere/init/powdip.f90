...
! with variable strength kappa, on a sphere.  The number density 
! of vortices of strength s is taken to be proportional to 1/s.

! The maximum strength of the vortices is chosen so that the
! fastest dipole propagates a mean inter-vortex spacing, 
! L = sqrt(4*pi/N), in unit time.  Here, N is the total number 
! of vortices.  Below, the dipole spacing a divided by L is 
! specified, while parameters.f90 contains N.

! Written on 16 July 2013 by D G Dritschel @ NY
...
write(*,*) ' Enter the ratio of maximum to minimum vortex strengths:'
read(*,*) rho
 ! Maximum vortex strength:
strmax=alp*ell**2

 ! Set vortex strengths:
sc=strmax
gamma=rho**(-one/dble(ndip-1))
sv(1)=sc
sv(2)=-sc
do j=2,ndip
  sc=sc*gamma
...
enddo
 ...
 ! Used below to ensure zero total angular momentum:
sum=zero
do i=1,n
  sum=sum+sv(i)**2
enddo
