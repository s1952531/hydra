...
character:: xvfile*12,cfr1*3,cfr2*3
...
write(*,*)
write(*,'(2(a,i4),a)') ' xvidi -dwidth ',ny,' -dheight ',nz,' '//xvfile//' &'
...
do iy=1,ny
  ...
  do iz=1,nz
    if (inside(iz,iy)) then
      ...
      bqq(ny*(nz-iz)+iy)=char(127+nint(120.d0*min(one,max(-one,qqt))))
    else
      bqq(ny*(nz-iz)+iy)=white
    endif
  enddo
enddo

 ! Write character pixel map for this frame:
write(21,rec=loop) (bqq(ii),ii=1,ngp)
