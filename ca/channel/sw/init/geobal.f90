program geobal
! Initialises a specific height field in near geostrophic balance.
...
double precision:: qq(0:ny,0:nxm1),bb(0:ny,0:nxm1),zz(0:ny,0:nxm1)
double precision:: aa(0:ny,0:nxm1)
double precision:: d10(0:nxm1),d21(0:nxm1),d32(0:nxm1)
double precision:: a,b,c,eps,fac,x,bcox,esix,y,yhat
double precision:: zavg,zadd
...
zz=zz-bb
 ...
 !Ensure that the domain average relative vorticity is consistent
 !with the mean boundary velocity:
call average(zz,zavg)
zadd=-sum(uu(ny,:)-uu(0,:))/(elly*dble(nx))-zavg
zz=zz+zadd

 !Calculate the potential vorticity:
qq=(zz+cof)/(one+hh)

 !Calculate the y component of the acceleration:
call yderiv(hh,bb)
bb=-cof*uu-csq*bb
 !Ensure zero boundary values:
bb(0 ,:)=zero
