...
subroutine urestore(qavg)

! First coarsens qa array to inversion grid, next finds the domain
! average, and finally restores the correct average, qavg.
...
double precision:: qu(0:nyu/2,0:nxu/2)
double precision:: qb(0:nyf,0:nxf)
double precision:: qf(0:nyf/2,0:nxf/2)
double precision:: qq(0:ny,0:nx)
double precision:: qavg0,qadd
integer:: nxh,nyh,ix,iy,mix,mixp1,mixm1,miy,miyp1,miym1

!---------------------------------------------------------------
 !***Warning: The coarsening assumes nxu = 4*nxf, nyu = 4*nyf
 !                        as well as nxf = 4*nx,  nyf = 4*ny
!---------------------------------------------------------------

 !Coarsen from the ultra-fine to the fine grid:
nxh=nxu/2
nyh=nyu/2

do ix=0,nxh
  mix=2*ix
  mixp1=nxu-abs(nxu-abs(mix+1))
  mixm1=nxu-abs(nxu-abs(mix-1))
  do iy=0,nyh
    miy=2*iy
    miyp1=nyu-abs(nyu-abs(miy+1))
    miym1=nyu-abs(nyu-abs(miy-1))
    qu(iy,ix)=f14*qa(miy,mix) &
...
do ix=0,nxf
  ...
  do iy=0,nyf
    ...
    qb(iy,ix)=f14*qu(miy,mix) &
         &  +f18*(qu(miy,mixm1)+qu(miy,mixp1)  &
         &       +qu(miym1,mix)+qu(miyp1,mix)) &
         & +f116*(qu(miym1,mixm1)+qu(miym1,mixp1)  &
         &       +qu(miyp1,mixm1)+qu(miyp1,mixp1))
  enddo
enddo

!-------------------------------------------------------------------
 !Coarsen from the fine grid to the inversion grid:
nxh=nxf/2
nyh=nyf/2
...
do ix=0,nxh
  ...
  do iy=0,nyh
    ...
    qf(iy,ix)=f14*qb(miy,mix) &
         &  +f18*(qb(miy,mixm1)+qb(miy,mixp1)  &
         &       +qb(miym1,mix)+qb(miyp1,mix)) &
         & +f116*(qb(miym1,mixm1)+qb(miym1,mixp1)  &
         &       +qb(miyp1,mixm1)+qb(miyp1,mixp1))
  enddo
enddo
...
do ix=0,nx
  ...
  do iy=0,ny
    ...
    miyp1=nyh-abs(nyh-abs(miy+1))
    miym1=nyh-abs(nyh-abs(miy-1))
    qq(iy,ix)=f14*qf(miy,mix) &
         &  +f18*(qf(miy,mixm1)+qf(miy,mixp1)  &
         &       +qf(miym1,mix)+qf(miyp1,mix)) &
         & +f116*(qf(miym1,mixm1)+qf(miym1,mixp1)  &
         &       +qf(miyp1,mixm1)+qf(miyp1,mixp1))
  enddo
enddo

!-------------------------------------------------------------------
 !Find average of qq and take away from qa:
