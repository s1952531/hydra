...
double precision:: daf(0:nx,0:ny)
...
fkmax=f23*sqrt(rkxmax**2+rkymax**2)
do kx=0,nx
  do ky=0,ny
    rk=sqrt(rkx(kx)**2+rky(ky)**2)
    if (rk .lt. fkmax) then 
      daf(kx,ky)=one
    else 
      daf(kx,ky)=zero
...
call ptospc_fs(nx,ny,ppi,pps,xfactors,yfactors,xtrig,ytrig)
 ...
 !Filter pp in spectral space using 2/3 rule:
do ky=0,ny
  do kx=0,nxm1  
    pps(kx,ky)=pps(kx,ky)*daf(kx,ky)
  enddo
enddo
