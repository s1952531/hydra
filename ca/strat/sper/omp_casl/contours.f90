...
!$omp do private(x1,y1,eta,x2,y2,avgsrc,xx,ix0,ix1,px,pxc,&
!$omp yy,iy0,iy1,py,pyc)
  do i=is,ie
    x1=xb(i)
    y1=yb(i)
    do k=1,ndiv
...
!$omp do private(xx,ix0,ix1,px,pxc,yy,iy0,iy1,py,pyc)
do i=1,nptq
  xx=glxi*(xq(i)-xmin)
  ix0=int(xx)
  ix1=ixp(ix0)
...
!$omp do private(ia,xx,ixdif,py0,ix)
do i=1,nptq
  ia=nextq(i)
  if (ia .gt. 0) then
...
!$omp do private(jump,ixbeg,sdq,ncr,ix,xx,px0,iy)
do i=1,nptq
  if (crossx(i)) then
    jump=sign(1,nxc(i))
...
!$omp parallel private(mix,mixp1,mixm1,miy,miyp1,miym1)
!$omp do 
do ix=0,nxh-1
  mix=2*ix
  mixp1=mix+1
