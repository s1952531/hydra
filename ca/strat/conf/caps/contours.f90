...
integer:: iyfw(0:nyu),iy0w(0:nyu),iy1w(0:nyu)
 ...
 !Fine-grid area weighting used in vorticity tendency calculation:
...
double precision,parameter:: gareaf=glxf*glyf
...
subroutine getzzsrc(bxsrc,t0,yox,yoy)
...
double precision:: bxsrc(0:ny,0:nx),bysrc(0:ny,0:nx)
double precision:: bxsrcf(0:nyf+1,0:nxf),bysrcf(0:nyf+1,0:nxf)
...
enddo
 !Double the edge values at x = xmin and xmax (by symmetry of b):
do iy=0,nyf
  bxsrcf(iy,  0)=two*bxsrcf(iy,  0)
  bxsrcf(iy,nxf)=two*bxsrcf(iy,nxf)
  bysrcf(iy,  0)=two*bysrcf(iy,  0)
  bysrcf(iy,nxf)=two*bysrcf(iy,nxf)
enddo
...
do ix=0,nx
  do iy=0,ny
    bxsrc(iy,ix)=bxsrc(iy,ix)*yoy(iy,ix)-bysrc(iy,ix)*yox(iy,ix)
  enddo
enddo
...
! Contour -> grid conversion.  The contours are represented by nodes
! (xq(i),yq(i)), i = 1, ..., nptq, where nextq(i) gives the index of
! the node following i, dq is the jump in q across all contours.
...
qbot(0)=zero
 !Corner value cannot be determined a priori; see calling routine
do ix=1,nxf
