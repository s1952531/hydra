...
nfr=int(raw_input('Frame to plot (1 for the first - default 1)? ') or 1)
xmax=float(raw_input('Maximum |x|,|y| to show (default 1.0)? ') or 1.0)
xmin=-xmax
ymax=xmax
ymin=-ymax
ntint=int(raw_input('Number of tick intervals on each axis (default 4)? ') or 4)
