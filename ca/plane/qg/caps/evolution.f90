...
gsave=abs(tgsave*dble(igrec)-tsim) .lt. 1.d-6*dt
csave=abs(tcsave*dble(icrec)-tsim) .lt. 1.d-6*dt
...
! Advances PV anomaly from time t to t+dt by a combination of contour 
...
!-------------------------------------------------------------------
 ...
 !Invert PV anomaly and compute velocity:
call inversion
...
call adapt
 ...
 !Calculate the source terms (sqs,sqd) for PV anomaly (qs,qd):
...
! Gets the source terms (sqs,sqd) for the PV anomaly (qs,qd)
...
! Inverts Laplace's operator on PV anomaly q to obtain the
! streamfunction (pp) ***in spectral space*** and the velocity 
...
qq=flo*(qs-qc)+qc+qd
 ...
 !Invert PV anomaly to obtain velocity field: 
...
if (btropic) then
 ...
 !Above, qq = PV anomaly in spectral space.
...
! Saves PV anomaly, energy and various spectra at the desired save time
...
!---------------------------------------------------------------
 !Write PV anomaly to qq.r4:
call spctop(nx,ny,qqs,wka,xfactors,yfactors,xtrig,ytrig)
write(31,rec=igrec+1) real(tgrid),real(wka)

 !Compute domain integral of q^2 and write to norms.asc:
...
write(83,rec=icrec) real(t),real(qa)
