...
!!!!!remove next line when congen doesn't reference it....
...
subroutine dynomega(uu,vv,pp,omega,epsb)
! Finds the rotation rate omega by minimizing the variance of the 
! normal velocity along the contours; also finds the steadiness
! parameter epsb, using streamfunction instead of Bernoulli pressure
! in the SW context.
...
double precision,parameter:: dp1=two*p1,dp2=two*p2,tp1=1.5d0*p1,tp2=1.5d0*p2
double precision:: dx(nptq),dy(nptq),a(nptq),b(nptq),c(nptq),d(nptq),e(nptq)
double precision:: u(nptq),v(nptq),f(nptq),g(nptq),q(nptq),r(nptq)
...
!-----------------------------------------------------------
 !Calculate omega from the ratio of sux=int(ds(u.n)(x.t)) to 
 !sxx=int(ds(x.t)**2) as well as other quantities needed to obtain
 !the "steadiness", epsb = sqrt((1/L)*int((pp-pbar)**2 ds))/pbar
 !where pbar = (1/L) int(pp ds) is the mean streamfunction and 
 !L = int(ds):
sux=zero
sxx=zero
do i=1,nptq
   !1st Gaussian point:
  eta1=p1*(a(i)+p1*(b(i)+p1*c(i)))
  deta1=a(i)+dp1*(b(i)+tp1*c(i))
   !(x,y):
  x1=xq(i)+p1*dx(i)-eta1*dy(i)
  y1=yq(i)+p1*dy(i)+eta1*dx(i)
   !(dx,dy):
  dx1=dx(i)-deta1*dy(i)
  dy1=dy(i)+deta1*dx(i)
   !(u,v) using bi-linear interpolation:
  xx=glxi*(x1-xmin)
  ix0=1+int(xx)
  pxc=dble(ix0)-xx
  px=one-pxc
  ...
  u1=pyc*(pxc*uu(iy0,ix0)+px*uu(iy0,ix1)) &
   & +py*(pxc*uu(iy1,ix0)+px*uu(iy1,ix1))

  v1=pyc*(pxc*vv(iy0,ix0)+px*vv(iy0,ix1)) &
   & +py*(pxc*vv(iy1,ix0)+px*vv(iy1,ix1))

  f(i)=pyc*(pxc*pp(iy0,ix0)+px*pp(iy0,ix1)) &
     & +py*(pxc*pp(iy1,ix0)+px*pp(iy1,ix1))

  u(i)=f12*(x1**2+y1**2)

   !2nd Gaussian point:
  eta2=p2*(a(i)+p2*(b(i)+p2*c(i)))
  deta2=a(i)+dp2*(b(i)+tp2*c(i))
   !(x,y):
  x2=xq(i)+p2*dx(i)-eta2*dy(i)
  y2=yq(i)+p2*dy(i)+eta2*dx(i)
   !(dx,dy):
  dx2=dx(i)-deta2*dy(i)
  dy2=dy(i)+deta2*dx(i)
   !(u,v) using bi-linear interpolation:
  ...
  u2=pyc*(pxc*uu(iy0,ix0)+px*uu(iy0,ix1)) &
   & +py*(pxc*uu(iy1,ix0)+px*uu(iy1,ix1))

  v2=pyc*(pxc*vv(iy0,ix0)+px*vv(iy0,ix1)) &
   & +py*(pxc*vv(iy1,ix0)+px*vv(iy1,ix1))

  g(i)=pyc*(pxc*pp(iy0,ix0)+px*pp(iy0,ix1)) &
     & +py*(pxc*pp(iy1,ix0)+px*pp(iy1,ix1))
  ...
  q(i)=ds1
  ai1=one/ds1
  xdt1=ai1*(x1*dx1+y1*dy1)
  udn1=ai1*(v1*dx1-u1*dy1)

  ds2=sqrt(dx2**2+dy2**2)
  r(i)=ds2
  ai2=one/ds2
  xdt2=ai2*(x2*dx2+y2*dy2)
  udn2=ai2*(v2*dx2-u2*dy2)

  sux=sux+ds1*udn1*xdt1+ds2*udn2*xdt2
  sxx=sxx+ds1*xdt1**2+ds2*xdt2**2
enddo
...
pbar=zero
do i=1,nptq
   !Rotating frame streamfunction at each Gaussian point:
  psi1=f(i)-omega*u(i)
  psi2=g(i)-omega*v(i)

   !Incremental arc lengths:
  ds1=q(i)
  ds2=r(i)

  arcl=arcl+ds1+ds2
  pint=pint+ds1*psi1+ds2*psi2
  ppint=ppint+ds1*psi1**2+ds2*psi2**2          
enddo

 !Gaussian Quadrature scaling:
arcl=f12*arcl
pint=f12*pint
ppint=f12*ppint
...
pbar=pint/arcl
 ...
 !Degree of unsteadiness (epsb):
epsb=sqrt(abs(ppint/(arcl*pbar**2)-one))
