...
double precision:: pi11(ng,ng),pi12(ng,ng),pi21(ng,ng),pi22(ng,ng)
double precision:: pm11(ng,ng),pm12(ng,ng),pm21(ng,ng),pm22(ng,ng)
...
double precision:: cop11,cop22,deni
double precision:: aa11,aa12,aa21,aa22
...
fsq=cof**2
 ...
 !Constants involving height and density ratios:
cop11= -f23*cona1    ! = -2*T_bar
cop22=     -cona3    ! =  6*(T_bar-2)
aa12= ahrsq*cona1    ! =  3*alpha*(H_2/H_1)^2*T_bar = (H_2/H_1)*(6-4*T_bar)
aa21=       cona2    ! =  3*(H_1/H_2)*T_bar
 !Note: mu_bar = alpha*H_2/H_1  &  T_bar = 6/(4+3*mu_bar)
  ...
  do kx=1,ng
    ...
    if (rks .gt. rkfsq) then
      ...
      ho22(kx,ky)= gw11(kx,ky)*deni
       !Non-hydrostatic pressure inversion operators:
      aa11=cop11-hbsq1*rks
      aa22=cop22-hbsq2*rks
      deni=one/(aa11*aa22-aa12*aa21)
      pi11(kx,ky)= aa22*deni   !L_11 in the notes
      pi12(kx,ky)=-aa12*deni   !L_12     "
      pi21(kx,ky)=-aa21*deni   !L_21     "
      pi22(kx,ky)= aa11*deni   !L_22     "
       !Linear operators (M_11 ... M_22) on l.h.s. of delta equations:
      pm11(kx,ky)=cop11*pi11(kx,ky)+aa12*pi21(kx,ky)
      pm12(kx,ky)=cop11*pi12(kx,ky)+aa12*pi22(kx,ky)
      pm21(kx,ky)=aa21*pi11(kx,ky)+cop22*pi21(kx,ky)
      pm22(kx,ky)=aa21*pi12(kx,ky)+cop22*pi22(kx,ky)
       !Semi-implicit inversion operators for delta and gamma_l:
      bb11(kx,ky)=pm11(kx,ky)*gw11(kx,ky)+pm12(kx,ky)*gw21(kx,ky)+ &
                  rdis(kx,ky)**2
      bb12(kx,ky)=pm11(kx,ky)*gw12(kx,ky)+pm12(kx,ky)*gw22(kx,ky)
      bb21(kx,ky)=pm21(kx,ky)*gw11(kx,ky)+pm22(kx,ky)*gw21(kx,ky)
      bb22(kx,ky)=pm21(kx,ky)*gw12(kx,ky)+pm22(kx,ky)*gw22(kx,ky)+ &
                  rdis(kx,ky)**2
...
double precision:: htot1(ng,ng),htot2(ng,ng),d1(ng,ng),d2(ng,ng)
double precision:: hd1(ng,ng),hd2(ng,ng),cc1(ng,ng),cc2(ng,ng)
...
hd2=htot2*d2
call dealias(hd2)
 !*** Below, htot_j denotes H_j*(1+h_1).

!---------------------------------------------------------------
 !Iteratively solve for hj, uj & vj in the two layers j = 1 & 2:

 !Compute cc1 = (1/3)*J(htot_1,htot_1*delta_1):
call jacob(htot1,hd1,cc1)
cc1=f13*cc1

 !Compute cc2 = J(htot_1,htot_1*delta_1+(1/2)*htot_2*delta_2) +
