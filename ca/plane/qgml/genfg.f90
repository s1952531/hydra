...
implicit integer(i-n)
 ...
 !Declarations:
real:: tr4,qdr4(ng,ng,nz)
character(len=3):: pind
double precision:: q0(ng,ng,nz)
...
open(40,file='cont/resi.r4',form='unformatted', &
...
open(40,file='cont/index'//pind,form='unformatted', &
      access='direct',status='old',recl=16*max(1,nq))
read(40,rec=1) npq(1:nq),i1q(1:nq),indq(1:nq),layq(1:nq)
close(40)

open(40,file='cont/nodes'//pind,form='unformatted', &
      access='direct',status='old',recl=16*max(1,nptq))
...
real tr4
 ...
 !Local arrays:
double precision:: qa(ngu,ngu,nz)
real:: rrqa(ngu/2,ngu/2,nz)
...
do iz=1,nz
  ...
  ! Bi-linear interpolate the residual qa to the fine grid and add to qa:
  do ix=1,ngu
    ixf=igfw(ix)
    ix0=ig0w(ix)
    ix1=ig1w(ix)
