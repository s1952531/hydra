...
subroutine recontour(qw)
! Main routine for recontouring (from D & Ambaum, 1996, QJRMS)
! qw: a gridded field added to that due to contours
...
if (nq .gt. 0) then
  ...
  call con2ugrid
   ...
   !Bi-linear interpolate the residual qw to the fine grid and add to qa:
  do ix=1,nxu
    ixf=ixfw(ix)
    ix0=ix0w(ix)
    ix1=ix1w(ix)
    ...
    do iy=1,nyu
      ...
      qa(iy,ix)=qa(iy,ix)+w00(iyf,ixf)*qw(iy0,ix0) &
                       & +w10(iyf,ixf)*qw(iy1,ix0) &
                       & +w01(iyf,ixf)*qw(iy0,ix1) &
                       & +w11(iyf,ixf)*qw(iy1,ix1)
  ...
  enddo
   ...
   !Check if field requires contouring by computing l1 norm of qw:
  call l1norm(qw,qwl1)
  if (qwl1 .lt. small) return

   !No contours: interpolate qw (which here contains the full field)
  ...
  do ix=1,nxu
    ...
    do iy=1,nyu
      ...
      qa(iy,ix)=w00(iyf,ixf)*qw(iy0,ix0)+w10(iyf,ixf)*qw(iy1,ix0) &
             & +w01(iyf,ixf)*qw(iy0,ix1)+w11(iyf,ixf)*qw(iy1,ix1)
