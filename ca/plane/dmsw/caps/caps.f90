!#########################################################################
!    The Doubly-Periodic Single-Layer Resistive Magneto-Hydrostatic
!          Shallow-Water Combined Lagrangian Advection Method
!#########################################################################

!       Code originally developed in July 2021 by D G Dritschel @ St Andrews.
!       Gilbert et al resistivity added 20 February 2023 by dgd @ St Andrews.

!       This code simulates the unforced Magneto-Hydrostatic Shallow-Water
!       Equations (MSWE) in variables (q_l,delta,gamma_l,b_x,b_y),
...
!       are the horizontal components of the magnetic field.  Here
!       h_tilde is the dimensionless height anomaly, c^2 = g*H
!       (H is the mean depth) and zeta is the relative vorticity
!       (dv/dx - du/dy).
...
!----------------------------------------------------------------------
 !Read in magnetic potential anomaly A and convert to spectral space as aas:
open(11,file='aa_init.r8',form='unformatted', &
      access='direct',status='old',recl=nbytes)
read(11,rec=1) t,zz
close(11)

call ptospc(ng,ng,zz,aas,xfactors,yfactors,xtrig,ytrig)
