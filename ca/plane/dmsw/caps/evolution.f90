...
! (for all spectral fields: qs, qd, ds, gs & aas).
...
double precision:: aasi(ng,ng),aasm(ng,ng),saas(ng,ng)
...
enddo
 ...
 !(sds,sgs,saas) for divergence, acceleration divergence & magnetic
 !potential anomaly (ds,gs,aas):
call source(sqs,sqd,sds,sgs,saas)
...
qd=disq*(qdm+dt4*sqd)-qdi
 ...
 !Update magnetic potential anomaly:
aasi=aas
aasm=aas+dt4*saas
aas=disb*(aasm+dt4*saas)-aasi
...
do iter=1,niter
  ...
  call inversion
   ...
   !Calculate the source terms (sqs,sqd) for PV (qs,qd) as well as those
   !(sds,sgs,saas) for the other fields:
  call source(sqs,sqd,sds,sgs,saas)
...
double precision:: wka(ng,ng),bx(ng,ng),by(ng,ng),hinv(ng,ng)
...
subroutine source(sqs,sqd,sds,sgs,saas)

! Gets the source terms (sqs,sqd) for the PV (qs,qd) as well as those
! (sds,sgs,saas) for divergence ds, acceleration divergence gs, and
! the magnetic potential anomaly (aas) --- all in spectral space.
...
! The fields ds, gs, qd, qs & aas are all spectrally truncated.
...
call spctop(ng,ng,wka,dd,xfactors,yfactors,xtrig,ytrig)
 ...
 !Get horizontal magnetic field in physical space as (bx,by):
wkp=one/(one+hh)
call dealias(wkp)
call gradient(aas,by,bx)
bx=wkp*(bx+bx0)
by=-wkp*by
call ptospc(ng,ng,bx,bxs,xfactors,yfactors,xtrig,ytrig)
call ptospc(ng,ng,by,bys,xfactors,yfactors,xtrig,ytrig)
bxs=filt*bxs
bys=filt*bys
wka=bxs
...
!---------------------------------------------------------------
 !Magnetic potential source:
call dealias(wkp) !wkp = h_tilde*u from above
wkp=uu+wkp
call dealias(wkq) !wkq = h_tilde*v from above
wkq=vv+wkq
 !Now (wkp,wkq) contains (h*u,h*v) in physical space

jz=eta*hh !jz = eta*h_tilde temporarily
call ptospc(ng,ng,jz,wka,xfactors,yfactors,xtrig,ytrig)
call gradient(wka,ux,uy) !(ux,uy) contains eta*grad{h}
jz=(wkp+ux)*by-(wkq+uy)*bx
call ptospc(ng,ng,jz,wka,xfactors,yfactors,xtrig,ytrig)
 !Apply de-aliasing filter:
saas=filt*wka
...
! Saves PV, energy, various spectra at the desired save time

implicit none

 !Local variables:
double precision:: htot(ng,ng),bx(ng,ng),by(ng,ng)
double precision:: bxs(ng,ng),bys(ng,ng)
double precision:: wka(ng,ng),wkb(ng,ng)
...
emag=f12*garea*sum(htot*(bx**2+by**2))
etot=ekin+epot+emag
