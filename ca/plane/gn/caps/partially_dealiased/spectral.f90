...
double precision:: bb(ng,ng),cc(ng,ng),dd(ng,ng),gg(ng,ng)
...
!-------------------------------------------------------
 ...
 !Compute Green-Naghdi term C = (H^2/3)*J(h,delta):
call jacob(hh,dd,cc)
cc=hbsq3*cc
call ptospc(ng,ng,cc,wka,xfactors,yfactors,xtrig,ytrig)
wka=filt*wka
call spctop(ng,ng,wka,cc,xfactors,yfactors,xtrig,ytrig)
...
do while (enorm .gt. tole)
  ...
  bb=hbsq3*htot*(badd+two*wkp)
   !De-aliase bb = B for use in NL products below:
  call ptospc(ng,ng,bb,wka,xfactors,yfactors,xtrig,ytrig)
  wka=filt*wka
  call spctop(ng,ng,wka,bb,xfactors,yfactors,xtrig,ytrig)
  ...
  wkc=wka-rksq*wkc
   !rksq = k^2+l^2 (spectrally filtered): - Laplace operator

   !Invert [c^2*grad^2-f*(f+<q>)]h = 
   !                f*(q+h*(q-<q>)-(H^2/3)*(1+h)*J(h,delta)) - gamma
   !                + Laplace((1+h)*B) + div(hx*B,hy*B):
  wkp=cof*(qq+hh*(qq-qbar)-htot*cc)-gg
  call ptospc(ng,ng,wkp,wkb,xfactors,yfactors,xtrig,ytrig)
   !wkb: spectral version of second line above
   !wkc: spectral version of third  line above
  ...
  htot=one+hh
   ...
   !Re-compute Green-Naghdi term C = (H^2/3)*J(h,delta):
  call jacob(hh,dd,cc)
  cc=hbsq3*cc
  call ptospc(ng,ng,cc,wka,xfactors,yfactors,xtrig,ytrig)
  wka=filt*wka
