...
dfac=drate*fpole
fac=dt*dfac
hyplat=one/(one+fac*(wmi*wave)**(2*nnu))

 !Longitudinal hyperviscous damping on wavenumber m for qd:
dfac=dt2*dfac
do m=1,nt
  fac=wmi*wave(m)
  do j=1,ng
    diss(j,m)=two/(one+dfac*(fac*clati(j))**(2*nnu))
  enddo
enddo
...
!-------------------------------------------------------------------
 !Initialise tri-diagonal arrays needed to invert height and for use
 !in determining delta in the balance conditions:
do m=1,nt
  k=ksig(m)
  sm=sigm(k)
  pm=psig(k)
...
! latitudinal derivative der.  Also used to determine delta in the
! balanced flow.
