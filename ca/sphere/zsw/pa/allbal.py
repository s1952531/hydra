...
#   here over a range of initial amplitudes A (see below)
#=================================================================
...
amp_in = input(' Enter A (default: 0.999)? ')
ampmax = float(amp_in or 0.999)

damp_in = input(' Increment in A to aid convergence (default: 0.001)? ')
damp = float(damp_in or 0.001)
...
ene_file = open('energy_g'+str(gamma)+'w'+str(wid)+'.asc','w')
...
while ddmax < 1.0:
   ...
   print("A = %5.3f,  Number of iterations = %5.0f" % (amp, kit))
   ...
   ene_file.write("%6.4f %15.12f %15.12f %15.12f \n" % (amp, ekin, epot, etot))

   if abs(amp-ampmax) < 1.e-7:
      break
...
if ddmax > 1.0:
   ...
   dfile='g'+str(gamma)+'w'+str(wid)+'A'+str(ampmax)+'.asc'
   out_file = open(dfile,'w')
   ...
   print()
   print(' Latitude displacement phi-a, scaled zonal velocity u/Omega,')
   print(' height h/H and PV q/(2*Omega) in ',dfile)
   print()
