...
double precision:: a(0:n),phi(0:n),phip(0:n),r(0:n),z(0:n)
...
!----------------------------------------------------------------------
 !Read in initial latitudes "a", masses, etc:
...
enddo
 ...
 !Determine equilibrium h from masses:
call convert

 !Store h(a)*cos(a) in h:
h=h*cos(phi)

 !Open and read displacement in phi:
open(21,file='davg.asc',status='old')
do j=0,n
  read(21,*) a(j),phi(j)
enddo
close(21)
