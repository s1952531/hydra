...
double precision:: d2lon(ng,nt),omv(ng,nt),dislon(ng,nt)
...
double precision:: simp(ng),sigm(2),psig(2)
...
double precision:: rlat,wm,wmi,fac,rkc,rkci,f283,weff,dfac
double precision:: a0d1,rsum,sm,pm,a0,b0,c0,d0,deti,cpla,dpla
integer:: k,j,m
...
do m=2,nt
  ...
  cmla(ng,m)=pc12-0.1d0*simp(ngm1)
...
double precision:: pm,utdb,vtdb,gsum,const
integer:: j,m
...
subroutine laplace(var)
! Calculates the Laplacian of var.
! *** var is assumed to be in physical space, and it is also returned
!     in physical space
...
!------------------------------------------------------------
call latder(var,wka)
 !wka = dvar/dlat; multiply by r = cos(lat) and take another derivative:
do i=1,nt
  do j=1,ng
    wka(j,i)=clat(j)*wka(j,i)
  enddo
enddo
call latder(wka,wkb)
 !wkb = d(r*dvar/dlat)/dlat

 !Compute two longitudinal derivatives; first convert var to spectral space:
call forfft(ng,nt,var,trig,factors)
call deriv(ng,nt,rk,var,wkc)
call deriv(ng,nt,rk,wkc,wka)
call revfft(ng,nt,wka,trig,factors)
!wka = d^2var/dlon^2 in physical space

 !Complete definition of Laplacian:
do i=1,nt
  do j=1,ng
    var(j,i)=clati(j)*(wkb(j,i)+clati(j)*wka(j,i))
  enddo
enddo
...
implicit none
 ...
 !Passed variables:
double precision:: var(ng,nt),vabs
 !Local variables:
