...
double precision,parameter:: glxu=twopi/dble(ntu),glyu=pi/dble(ngu)
 ...
 !Basic constants:
double precision,parameter:: dl=twopi/dble(nt),dli=dble(nt)/(twopi+small),hpidl=(pi+dl)/two
...
double precision,parameter:: dmi=two/dm,d4small=small*dl**4
 ...
 !Logicals:
logical,parameter:: stoch=(esr .gt. zero),thermal=(rtherm .gt. zero),friction=(rekman .gt. zero)
logical,parameter:: topogr=((atopo .ne. zero) .or. (btopo .ne. zero))
