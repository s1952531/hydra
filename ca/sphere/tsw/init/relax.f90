...
double precision:: eps,phi0,b,amp,rlat,fm,acmlon,binv,phi,phin,phis
double precision:: rsum,rsumi,dsumi,vsum
integer:: i,j,m

if (.not. thermal) then
  write(*,*) ' This can only be used when there is thermal damping, i.e.'
  write(*,*) ' when the thermal damping rate alpha > 0.'
  write(*,*)
  write(*,*) ' *** STOPPING ***'
  write(*,*)
  stop
...
close(20)
 ...
 !Write zero initial log potential temperature field:
hh=zero
...
!------------------------------------------------------------------------------
 !Compute and write thermal equilibrium height field, h_e:
fm=dble(m)
eps=f12*eps
binv=one/b
do i=1,nt
...
enddo
 ...
 !Restore global mean h_e to be 1 (recall h_e = H_e/H_char):
vsum=zero
do i=1,nt
