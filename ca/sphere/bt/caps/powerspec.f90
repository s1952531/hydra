program powerspec
!  -------------------------------------------------------------------------
!  |  Computes the 1D power spectrum for vorticity from data in zz.r4,     |
...
integer, parameter:: lmaxc=ng/2-1

real:: tr4,qqr4(ng,nt)
double precision:: qq(ng,nt),qq0(ng,nt)

 !Half-grid -> Full-grid tri-diagonal arrays:
double precision:: utd(nt),xtd(nt),etd(nt),htd(nt),ptd(nt),xndeno,xend

 !Spectral arrays:
double precision:: cilm(2,ng+1,ng+1),ss(ng+1),lwv(ng)
...
do  
  ...
  write(20,'(f12.5)') tr4
  do l=1,lmaxc
    write(20,'(2(1x,f12.8))') lwv(l),log10(ss(l+1)+1.d-32)
  enddo
...
write(*,*)
write(*,*) ' The vorticity spectra are in spectra.asc'
