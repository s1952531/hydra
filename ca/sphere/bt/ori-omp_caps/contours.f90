...
double precision:: clatu(ngu-1),dsumui

 !For removing average vorticity in this module:
double precision:: clatc(ng),dsumci
...
do j=1,ng
  clatc(j)=cos((dble(j)-f12)*dl-hpi)
enddo

rsumc=f1112*(clatc(1)+clatc(ng))
do j=2,ngm1
  rsumc=rsumc+clatc(j)
enddo
 !Note, rsumc = 1/sin(dl/2) - sin(dl/2)/6 exactly.
rsumci=one/rsumc
...
!$omp do private(ri,i,ip1,bbl,abl,rj,j,jp1,cbl,dbl,ulon,ulat,rr,rri,zula)
do k=1,npt
...
!$omp do private(jump,ioff,ncr,i,rlatc,j,p)
do k=1,npt
  if (ntc(k) .ne. 0) then
    jump=sign(1,ntc(k))
