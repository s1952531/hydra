...
call laplinv(qq,wka,wkb)
 !Here the streamfunction psi is wka while wkb = d(psi)/dlat.
...
umax=zero
zmax=zero
!$omp do reduction(max:umax,zmax)
do i=1,nt
  do j=1,ng
    umax=max(umax,wka(j,i))
    zmax=max(zmax,wkb(j,i))
  enddo
enddo
!$omp enddo
!$omp single
umax=sqrt(umax)+small
zmax=sqrt(zmax)+small
!$omp end single
...
avqq=f1112*(qq(1,1)*clat(1)+qq(ng,1)*clat(ng))
do j=2,ngm1
  avqq=avqq+qq(j,1)*clat(j)
enddo
avqq=avqq*rsumi
do j=1,ng
  qq(j,1)=qq(j,1)-avqq
