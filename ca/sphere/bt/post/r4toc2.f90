program r4toc2
!  ---------------------------------------------------------------------
!  |   Creates character maps for producing images of relative or      |
!  |   absolute vorticity either at a fixed time (using data in the    |
!  |   grid subdirectory or in the fine subdirectory), or over a       |
!  |   selected range of times (using data in the main job directory). |
!  |                                                                   |
...
integer,parameter:: mgv=16,ntv=mgv*nt, ngv=mgv*ng
 !mgv:       Ultra-fine conversion grid ratio (same as mgu in post/grid.f90)
...
character(len=2),dimension(2),parameter:: ivar=['zz','zz']
 !Output variable names (in data files created):
character(len=2),dimension(2),parameter:: ovar=['zz','za']
...
if (omega .ne. zero) then
  ...
  write(*,*)
  write(*,*) '    (1)  relative vorticity, zeta'
  write(*,*) ' or (2)  absolute vorticity, zeta+f'
  read(*,*) iopt
  ...
  if (iopt .lt. 1 .or. iopt .gt. 2) then
   ...
   !No rotation; we simply image vorticity:
  iopt=1
endif

if (sopt .lt. 3) then
   !Data are read from zz.r4 either in the current directory (sopt = 1) 
   !or in the grid subdirectory (sopt = 2)
  nx=nt
  ny=ng
  ...
  nx=ntv
  ny=ngv
   !Here the data dimensions are ntv longitudes and ngv latitudes
endif
...
if (sopt .eq. 3) then
   !This is for imaging the ultra-fine grid vorticity field:
  ...
  open(11,file='fine/qq'//suffix//'.r4',form='unformatted',status='old')
...
if (fopt .eq. 1) then
  call llgrid(ny,nx,kbeg,kend,kint,iopt,sopt)
else
...
subroutine llgrid(ny,nx,kbeg,kend,kint,iopt,sopt)
...
real:: t,dlat,aqqmin,aqqmax,bqqmin,bqqmax
...
do k=kbeg,kend,kint
  ...
  if (sopt .lt. 3) then
    ...
    read(11) t,qq(1:ny,1:nx)
  endif

   !If absolute vorticity, add f:
  if (iopt .eq. 2) then
    dlat=pi/float(ny)
    do j=1,ny
      cof(j)=fpole*sin((float(j)-f12)*dlat-hpi)
    enddo
    do i=1,nx
      do j=1,ny
        qq(j,i)=qq(j,i)+cof(j)
