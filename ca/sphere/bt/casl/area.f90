...
! Also computes the mean latitude and longitude of each level.              !
!                                                                           !
! Writes area.asc listing t vs area(l), l = 1, ..., l_max                   !
! as well as latitude.asc & longitude.asc in the same format.               !
!                                                                           !
...
double precision,parameter:: bsq=asp**2,ombsq=one-bsq,pdiv=f13/ombsq
...
call getlev(n,npt)
 ...
 !Open output files:
open(21,file='area.asc',status='replace')
open(22,file='latitude.asc',status='replace')
open(23,file='longitude.asc',status='replace')
...
write(*,*) ' See also latitude.asc and longitude.asc for mean lat/lon.'
write(*,*)
...
double precision:: area(2*nlevm+1)
 !Centres:
double precision:: xcen(2*nlevm+1),ycen(2*nlevm+1),zcen(2*nlevm+1)
...
double precision:: x1(npt),y1(npt),z1(npt),dx1,dy1,dz1(npt),dlon1(npt)
double precision:: x2(npt),y2(npt),z2(npt),dx2,dy2,dz2(npt),dlon2(npt)
double precision:: bc,pref,q,cth,mu0,mu1,mu2,sarea,pp1,pp2,qdz1,qdz2
...
do i=1,npt
  ...
  ddel=f12-p1
  x1(i)=x(i)+p*dx(i)+eta*ax+del*sx
  y1(i)=y(i)+p*dy(i)+eta*ay+del*sy
  z1(i)=z(i)+p*dz(i)+eta*az+del*sz
  dx1=dx(i)+deta*ax+ddel*sx
  dy1=dy(i)+deta*ay+ddel*sy
  dz1(i)=dz(i)+deta*az+ddel*sz
  dlon1(i)=(x1(i)*dy1-y1(i)*dx1)/(x1(i)**2+y1(i)**2)
   !2nd Gaussian point:
  eta=p2*(a(i)+p2*(b(i)+p2*c(i)))
  del=f12*p2*(one-p2)
  deta=a(i)+tp2*(b(i)+ttp2*c(i))
  ddel=f12-p2
  x2(i)=x(i)+p*dx(i)+eta*ax+del*sx
  y2(i)=y(i)+p*dy(i)+eta*ay+del*sy
  z2(i)=z(i)+p*dz(i)+eta*az+del*sz
  dx2=dx(i)+deta*ax+ddel*sx
  dy2=dy(i)+deta*ay+ddel*sy
  dz2(i)=dz(i)+deta*az+ddel*sz
  dlon2(i)=(x2(i)*dy2-y2(i)*dx2)/(x2(i)**2+y2(i)**2)
enddo

 !Calculate area etc depending on aspect ratio of ellipsoid:
if (asp .lt. one) then
   !Oblate ellipsoid:
  bc=sqrt(abs(ombsq))
  pref=bsq/bc
  ...
  do j=1,n
    ...
    do i=i1(j),i2(j)
      ...
      xcen(l)=xcen(l)+y1(i)*qdz1+y2(i)*qdz2
      ycen(l)=ycen(l)-x1(i)*qdz1-x2(i)*qdz2
      zcen(l)=zcen(l)+pp1*dlon1(i)+pp2*dlon2(i)
    enddo
  enddo
  ...
  do j=1,n
    ...
    do i=i1(j),i2(j)
      ...
      xcen(l)=xcen(l)+y1(i)*dz1(i)+y2(i)*dz2(i)
      ycen(l)=ycen(l)-x1(i)*dz1(i)-x2(i)*dz2(i)
      zcen(l)=zcen(l)+f12*(one-z1(i)**2)*dlon1(i)+f12*(one-z2(i)**2)*dlon2(i)
    enddo
  enddo
endif
 ...
 !Normalise areas:
do l=1,nlev
  xcen(l)=xcen(l)/area(l)
  ycen(l)=ycen(l)/area(l)
  zcen(l)=zcen(l)/area(l)
  area(l)=f12*area(l)
enddo
...
write(21,'(1x,f12.5,10(1x,f14.9))') t,(area(l),l=1,nlev)
write(22,'(1x,f12.5,10(1x,f14.9))') t,(asin(zcen(l)),l=1,nlev)
write(23,'(1x,f12.5,10(1x,f14.9))') t,(atan2(ycen(l),xcen(l)),l=1,nlev)
