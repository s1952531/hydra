!#########################################################################
!               The Doubly-Periodic Vertically-Averaged
!                 Shallow-Water Pseudo-Spectral Method
!#########################################################################

!       Code adapted from ~/hydra/ps/plane/sw & gn codes in August 2019
!       by D G Dritschel @ St Andrews.

!       This code simulates the unforced vertically-averaged non-hydrostatic
!       shallow-water equations in variables (ql,delta,gamma), where ql is
!       the linearised potential vorticity anomaly, delta is the velocity
!       divergence, and gamma is the linearised acceleration divergence.
!       Note: ql = zeta - f*h and gamma = f*zeta - c^2*lap(h), where zeta 
!       is the relative vorticity, f is the Coriolis frequency and h is the 
!       dimensionless height anomaly.  c^2 = g*H where H is the mean height.

!       The full algorithm consists of the following modules:

!       vaps.f90      : This source - main program to evolve fields;
...
double precision:: dd(ng,ng),ppn(ng,ng),rx(ng,ng),ry(ng,ng)
...
enddo
 ...
 !Store rhs of pressure iteration, -gs, in wkb for use in divergence
 !tendency below:
wkb=wkg-gs+wkb
...
call ptospc(ng,ng,wkp,wkf,xfactors,yfactors,xtrig,ytrig)
 !wkf is needed below for the gamma source as well.
