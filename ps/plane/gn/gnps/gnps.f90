...
call divs(wkp,wkq,wke)
 !***Do not overwrite wke until sqs is determined shortly below.
...
call jacob(uu,vv,wkp)
 !***Do not overwrite wkp until ds source is found below

 !Form gamma_tilde = gamma + 2*(J(u,v) - delta^2):
wkq=two*(wkp-dd**2)
 !Convert to spectral space, add on gs to get spectral gamma_tilde (in wka):
call ptospc(ng,ng,wkq,wka,xfactors,yfactors,xtrig,ytrig)
wka=filt*wka+gs
 !Return to physical space as aa:
call spctop(ng,ng,wka,aa,xfactors,yfactors,xtrig,ytrig)
 !***Do not overwrite aa (gamma_tilde) until iteration below to find gamma_t

 !Compute (H^2/3)*(1+h)*J(gamma_tilde,h) and store in wkf (spectral):
call jacob(aa,hh,wkz)
 !Filter J(gamma_tilde,h) before multiplying it by (1+h):
call dealias(wkz)
 !Finish q_l source:
wkz=hbsq3*htot*wkz
call ptospc(ng,ng,wkz,wkf,xfactors,yfactors,xtrig,ytrig)
sqs=filt*(wkf-wke)
...
wkp=hh*uu
wkq=hh*vv
 !Compute div(h*u,h*v) = -N_h spectrally and filter for use shortly below:
...
call divs(wkp,wkq,wkd)
 !Add wkf = (H^2/3)*(1+h)*J(gamma_tilde,h) to -div... and spectrally truncate:
wkd=filt*(wkf-wkd)
 !Convert to physical space as ztn = N_zeta:
...
wkp=f12*(uu**2+vv**2)
 !wkp = K = (u^2+v^2)/2
call ptospc(ng,ng,wkp,wka,xfactors,yfactors,xtrig,ytrig)
 !Spectrally truncate K then calculate its x & y derivatives:
wka=filt*wka
call xderiv(ng,ng,hrkx,wka,wkd)
call spctop(ng,ng,wkd,wkp,xfactors,yfactors,xtrig,ytrig)
 !wkp is K_x in physical space
call yderiv(ng,ng,hrky,wka,wkd)
call spctop(ng,ng,wkd,wkq,xfactors,yfactors,xtrig,ytrig)
 !wkq is K_y in physical space
...
call ptospc(ng,ng,wkh,wka,xfactors,yfactors,xtrig,ytrig)
 !Spectrally truncate wka = (1+h)^3*gamma_tilde then calculate its
 !x & y derivatives:
wka=filt*wka
call xderiv(ng,ng,hrkx,wka,wkd)
...
call dealias(wkm)
 !Absolute vorticity:
wkz=zz+cof
 !Add up all terms to complete calculation of u_t and v_t:
ut=wkm*wkg-wkp+wkz*vv-csq*hx
call dealias(ut) !Need de-aliasing to compute J(u_t,v) below
vt=wkm*wkh-wkq-wkz*uu-csq*hy
call dealias(vt) !Need de-aliasing to compute J(u,v_t) below

 !Obtain B = 2*(1+h)*(J(u_t,v)+J(u,v_t)-2*delta*delta_t) in the array bb:
call jacob(ut,vv,wkp)
call jacob(uu,vt,wkq)
 !Form J(u_t,v)+J(u,v_t)-2*delta*delta_t then de-alias:
bb=wkp+wkq-two*dd*ddt
call dealias(bb)
 !Multiply this by 2*(1+h) to complete definition of B and de-alias:
bb=two*htot*bb
call dealias(bb)
...
wkp=two*ht*wkf+htot*bb ! wkf = (1+h)*gamma_tilde here (de-aliased)
call ptospc(ng,ng,wkp,wka,xfactors,yfactors,xtrig,ytrig)
 !Apply spectral Laplacian, -(k^2+l^2), to wka and add to 
 !wke = -c^2*Lap(N_h) computed above:
wka=cof*wkb+wke-hbsq3*rksq*wka !This is fully de-aliased

 !Obtain (H^2/3)*div(px,py) where px = gamma_tilde*((1+h)*h_t)_x+B*h_x
 !                            and py = gamma_tilde*((1+h)*h_t)_y+B*h_y
wkp=htot*ht
call ptospc(ng,ng,wkp,wkc,xfactors,yfactors,xtrig,ytrig)
 !Spectrally truncate wkc = (1+h)*h_t before differentiating wrt x & y:
wkc=filt*wkc
