...
jmin=-int(-umin/du)
jmax= int( umax/du)
clevels3=np.linspace(du*float(jmin),du*float(jmax),jmax-jmin+1)
du=du/mult
jmin=-int(-umin/du)
jmax= int( umax/du)
clevels3f=np.linspace(du*float(jmin),du*float(jmax),jmax-jmin+1)

#==============================================================================
# Set up figure:

fig, (ax1, ax2, ax3) = plt.subplots(figsize=[14,9], nrows=3, ncols=1)
...
ax1.yaxis.set_ticks([-0.5,-0.4,-0.3,-0.2,-0.1,0.0])
ax2.yaxis.set_ticks([-0.5,-0.4,-0.3,-0.2,-0.1,0.0])
ax3.yaxis.set_ticks([-0.5,-0.4,-0.3,-0.2,-0.1,0.0])
ax1.set_yticklabels([r'$-0.5$',r'$-0.4$',r'$-0.3$',r'$-0.2$',r'$-0.1$',r'$0$'],fontsize=20)
ax2.set_yticklabels([r'$-0.5$',r'$-0.4$',r'$-0.3$',r'$-0.2$',r'$-0.1$',r'$0$'],fontsize=20)
ax3.set_yticklabels([r'$-0.5$',r'$-0.4$',r'$-0.3$',r'$-0.2$',r'$-0.1$',r'$0$'],fontsize=20)

# Fine-tune figure; hide x ticks for upper plots
plt.setp(ax1.get_xticklabels(), visible=False)
plt.setp(ax2.get_xticklabels(), visible=False)

ax1.set_title('$b(0,y,z)$', fontsize=20)
ax2.set_title('$\\zeta(0,y,z)$', fontsize=20)
ax3.set_title('$u(0,y,z)$', fontsize=20)
...
extent=(-np.pi,np.pi,-0.5,0.0)

# Plot the image in an array with an optional colourbar:
im1=ax1.imshow(b,cmap=cm.seismic,vmin=bmin,vmax=bmax,extent=extent,origin='lower',interpolation='bilinear',aspect='auto')
cs1=ax1.contour(b, clevels1f, colors='k', extent=extent)
divider = make_axes_locatable(ax1)
cax1 = divider.append_axes("right", size="4%", pad=0.1)
cbar=fig.colorbar(im1, cax=cax1, ticks=clevels1)
cbar.add_lines(cs1)

im2=ax2.imshow(z,cmap=cm.seismic,vmin=zmin,vmax=zmax,extent=extent,origin='lower',interpolation='bilinear',aspect='auto')
cs2=ax2.contour(z, clevels2f, colors='k', extent=extent)
divider = make_axes_locatable(ax2)
cax2 = divider.append_axes("right", size="4%", pad=0.1)
cbar=fig.colorbar(im2, cax=cax2, ticks=clevels2)
cbar.add_lines(cs2)

im3=ax3.imshow(u,cmap=cm.seismic,vmin=umin,vmax=umax,extent=extent,origin='lower',interpolation='bilinear',aspect='auto')
cs3=ax3.contour(u, clevels3f, colors='k', extent=extent)
...
#=========================================================================
# Save image:
fig.savefig('cross.eps', format='eps', dpi=300)
