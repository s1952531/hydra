...
jopt=int(raw_input(' Compare (1) SW & 3D or (2) GN & 3D (default 1): ') or 1)
print
if jopt == 1:
   dir2d=dirsw
   label2d='SW'
...
if iopt == 1:
   ...
   in_file2d=open(dir2d+'hh.r4','r')
   title_label='$\\tilde{h}$'
   field='h'
elif iopt == 2:
   in_file3d=open(dir3d+'zeta.r4','r')
   in_file2d=open(dir2d+'zz.r4','r')
   title_label='$\\zeta$'
   field='zeta'
elif iopt == 3:
   in_file3d=open(dir3d+'d.r4','r')
   in_file2d=open(dir2d+'dd.r4','r')
   title_label='$\\delta$'
   field='delta'
elif iopt == 4:
   in_file3d=open(dir3d+'gt.r4','r')
   in_file2d=open(dir2d+'gt.r4','r')
   title_label='$\\tilde{\\gamma}$'
   field='gamma-tilde'
else:
   in_file3d=open(dir3d+'pn.r4','r')
   in_file2d=open(dir2d+'pn.r4','r')
   title_label='$P_n$'
   field='pn'
...
in_file=open(dir2d+'ecomp.asc','r')
...
# Add titles:
ax1.set_title('3D', fontsize=30, fontname='Times New Roman')
ax2.set_title(label2d, fontsize=30, fontname='Times New Roman')
ax3.set_title(labeldiff, fontsize=30, fontname='Times New Roman')
...
raw_array=np.fromfile(in_file2d,dtype=np.float32)
in_file2d.close()
...
# Work out the overall min/max values for difference field:
zmin3=np.amin(Z3)
zmax3=np.amax(Z3)

# Make limits the same and centre on 0:
...
#=========================================================================
# Save figure:
outfile=label2d+field+'_H{x:.1f}'.format(x=hbar)+'n'+str(ng)+'t{x:.0f}'.format(x=t)+'.eps'
