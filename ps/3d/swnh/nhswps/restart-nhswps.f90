...
!       *** Use for re-starting a simulation from a selected time ***
...
integer:: iz,loop,iread,nreads,m
...
gs(1,1,:)=zero
 ...
 !Read non-hydrostatic pressure:
read(36,rec=loop) tr4,q3dr4
pn=dble(q3dr4)
...
!--------------------------------------
 !Open all plain text diagnostic files and read to current time:
open(16,file='ecomp.asc',status='old')
td=zero
nreads=0
do
  iread=0
...
backspace(16)
if (iread .eq. 0) then
  write(*,*) ' Read ecomp.asc up to time before t = ',td
else
  write(*,*) ' Read ecomp.asc up to time t = ',td
endif
...
ds=sgs+dt2i*sds      !2*T_tilde_delta
wka=zero
wkb=zero
do iz=0,nz
...
do iz=0,nz
  ...
  gs(:,:,iz)=wkb-fsq*sds(:,:,iz)+dt2i*sgs(:,:,iz)  !2*T_tilde_gamma
enddo
wka=zero
do iz=0,nz
