...
frame=int(raw_input(' Frame to show (1, 2, ...; default 1)? ') or 1)
yam=int(raw_input(' Max |y| to show (default 1)? ') or 1.0)
cbopt=str(raw_input(' Add a colourbar (default n)? ') or 'n')

# Read data:
in_file=open('../3d/swnh/ng256ld0.5r001/2d/q.r4','r')
...
if cbopt == 'n':
   ax.imshow(Z[iy1:iy2,:],cmap=cm.terrain,vmin=zmin,vmax=zmax,extent=(-np.pi,np.pi,-yam,yam),origin='lower',interpolation='bilinear')
else:
   im=ax.imshow(Z[iy1:iy2,:],cmap=cm.terrain,vmin=zmin,vmax=zmax,extent=(-np.pi,np.pi,-yam,yam),origin='lower',interpolation='bilinear')
...
#=========================================================================
# Save image:
fig.savefig('pv'+str(frame)+'.eps', format='eps', dpi=dpi, bbox_inches = 'tight')

print
print ' To view the image, type'
print
print ' gv pv'+str(frame)+'.eps'
print
